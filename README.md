...
- Construye un perfil **quíntico (polinomio de quinto grado)** por eje (x, y, z), que garantiza una trayectoria suave.

En cada instante `t`, el nodo evalúa:
- Posición deseada: `p_d(t)`
- Velocidad deseada: `v_d(t)`
- Aceleración deseada: `a_d(t)`
...
- Publica referencia completa en:
  - `/simple_drone/ref_odom` (`nav_msgs/Odometry`), con la convención:
    - `pose.pose.position` → `p_d` (posición deseada en mundo)
    - `twist.twist.linear` → `v_d` (velocidad deseada en mundo)
    - `twist.twist.angular` → `a_d` (aceleración deseada en mundo, usada como contenedor temporal)
...
Este nodo implementa un **controlador LQR en espacio de estados** que actúa como **lazo externo de aceleración/velocidad**, cerrando el seguimiento de una referencia completa formada por posición, velocidad y aceleración deseadas (`p_d`, `v_d`, `a_d`) generada por el nodo `trajectory_follower` en modo `lqr`.
...
El controlador aplica la siguiente ley de control:

- aceleración_deseada = aceleración_referencia - K * [error_posicion; error_velocidad]

donde:
- error_posicion = posicion_actual - posicion_deseada
- error_velocidad = velocidad_actual - velocidad_deseada
- K es una matriz de ganancias LQR de tamaño 3x6 precalculada
...
- extracción del yaw desde el cuaternión
...
- Integración explícita:
  - derivada de v = aceleración_deseada
- Saturaciones duras:
  - aceleración máxima
  - velocidad máxima
